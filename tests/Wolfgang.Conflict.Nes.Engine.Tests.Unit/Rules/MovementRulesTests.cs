...
public class MovementRulesTests
{
    ...
    public void GetReachable_returns_full_radius_on_open_plains()
    {
        ...
        var tank = Unit.FullStrength(new UnitId(1), Side.Blue, UnitKind.Tank, new HexCoord(4, 1));
    ...
    public void GetReachable_excludes_hexes_blocked_by_other_units()
    {
        ...
        var blocker = Unit.FullStrength(new UnitId(2), Side.Red, UnitKind.Infantry, new HexCoord(1, 0));
    ...
    public void FindPath_returns_null_for_unreachable_destination_over_budget()
    {
        ...
        var infantry = Unit.FullStrength(new UnitId(1), Side.Blue, UnitKind.Infantry, new HexCoord(0, 0));
    ...
    {
        var map = AllPlains(6, 3);
        var tank = Unit.FullStrength(new UnitId(1), Side.Blue, UnitKind.Tank, new HexCoord(2, 1));
        var state = MakeState(map, tank);
    ...
    {
        var map = AllPlains(6, 3);
        var tank = Unit.FullStrength(new UnitId(1), Side.Blue, UnitKind.Tank, new HexCoord(2, 1)) with { MovesRemaining = 0 };
        var state = MakeState(map, tank);
    ...
    [Fact]
    public void GetReachable_returns_empty_for_unit_with_no_fuel()
    {
        var map = AllPlains(6, 3);
        var tank = Unit.FullStrength(new UnitId(1), Side.Blue, UnitKind.Tank, new HexCoord(2, 1)) with { Fuel = 0 };
        var state = MakeState(map, tank);
